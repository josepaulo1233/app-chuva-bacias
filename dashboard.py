...
#fig = gera_mapa_latlon_psat(my_txt_files_name[-1])
...
for filename in my_txt_files:

    dff = pd.read_table(filename, delim_whitespace=True, header=None)
...
def gera_graf(df, nome_bacia):
    
    df = pd.DataFrame(df, index = daterange)
    ...
    graf.update_xaxes(tick0=daterange[0], dtick="d1", tickformat="%d/%b")
    ...
    graf.update_coloraxes(colorbar_title=None)
    
    return graf

grande = gera_graf(values_grande, 'grande')
paranaiba = gera_graf(values_paranaiba, 'paranaiba')
tocantins = gera_graf(values_tocantins, 'tocantins')
saofrancisco = gera_graf(values_saofrancisco, 'são francisco')
paranapanema = gera_graf(values_paranapanema, 'paranapanema')
parana = gera_graf(values_parana, 'parana')
iguacu = gera_graf(values_iguacu, 'iguaçu')
uruguai = gera_graf(values_uruguai, 'uruguai')
